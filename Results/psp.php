...
{
$dir=scandir("exams/psp/");if(!$dir)echo"error scanning";
foreach($dir as $val)
{$id=substr("$val",0,7);
$sub=substr("$val",8);
if($val==$id."_".$sub)
{
	$a="exams/psp/".$id."_".$sub.".txt";
	$f=fopen($a,'a+');
	$a1="exams/psp/".$id."_".$sub;
	$f1=fopen($a1,'r');
	$a2="exams/psp/".$id."_".$sub.".txt";
	$f2=fopen($a2,'a');
while(!feof($f1))
{
$x=fgetc($f1);
...
if($subj=='psp')
{$qq2="insert into $sub values('$id',$a,$b,$c,$d,$e,$f,$g,$h,$i,$j,$k,$l,'0')";}
else
{$qq2="insert into $sub values('$id','$a','$b','$c','$d','$e','$f','$g','$h','$i','$j','0')";}
$qq3=mysql_query("$qq2");
if(!$qq3)echo mysql_error();
$ch=fwrite($final,$a.$b.$c.$d.$e.$f.$g.$h.$i.$j.$k.$l);
if($ch)
fclose($final);
fclose($q1);fclose($f3);
//unlink("exams/B082035_PSP.txt");
unlink("exams/psp/".$id."_".$sub.".txt");
//unlink("exams/B082035_PSP_FINAL.TXT");
unlink("exams/psp/".$id."_".$sub."_FINAL.TXT");
if($qq3){unlink("exams/psp/".$id."_".$sub);unlink("exams/psp/".$id."_".$sub.".TXT");}
...
$query="update `week`.`TOTAL` SET `psp`='$count' where `id`='$id';";
$query1=mysql_query($query);
if($query1)unlink("exams/psp/".$id."_".$sub.".TXT");
}
}
}
else
